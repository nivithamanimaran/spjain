...
const CurrentExam = () => {
  ...
  const columns = [
    ...
    // },
    {
      ...
      Cell: ({ value, row }) => {
        return (
          <>
            {console.log('rowrowrowrow', row.original.InvReqStatus, value)}
  ...
  const postCurruntInviExamantionAccept = async id => {
    try {
      ...
      const res = await postCurruntInviExamantionFacAccept(
      ...
      // alert(res);
      getCurruntInviExamantionFacultyData();
    } catch (error) {
  ...
  const getCurruntInviExamantionFacultyData = async () => {
    try {
      setLoading(true);
        ...
        res.length === 0
          ...
          : res?.data[0]?.ExamDetails?.map(item => {
              return {
                ...
                Date: item.exmdate,
                Time: item.exmTime,
                isEligible: item.InvReqStatus === 'Accept' ? true : false,
