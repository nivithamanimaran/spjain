...
export default function Retest(props) {
  const classes = useStyles();
        ...
        return (
          <>
            ...
            >
              ...
              >
                Verify the all details before apply for Retest{' '}
              </DialogTitle>
  ...
  const getReTestData = () => {
    ...
    getRetestReval(object.ContactId)
      .then(res => {
        const reEvalinfo =
          res[0]?.reTestList === null
            ? []
            : res[0]?.reTestList?.map(item => {
                return {
