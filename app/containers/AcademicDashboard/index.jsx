...
export default function AcademicDashboard(props) {
  ...
  React.useEffect(() => {
    ...
    const subjectBar = [];
    const newData = [];
    Object.entries(AcademicData.StudentDashboard).map((item, keyVal) => {
      academicBar.push({
        academicItem: item[0],
        data: item[1],
      });
  ...
  const markAssessment = (el, index) => {
    let tempData = [];
    academicBar.map(item => {
      if (item.academicItem === el.academicItem) {
        tempData.push(item);
      }
    });
    tempData[0].data[index].check = !tempData[0].data[index].check;
    setAcademicBar(tempData);
  };
  return (
