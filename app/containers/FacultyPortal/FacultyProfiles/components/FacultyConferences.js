...
const FacultyConferences = ({}) => {
  ...
  const [
    facultyConferencesDetails,
    setFacultyConferencesDetails,
  ] = React.useState([
    {
      res: {
        attributes: {
  ...
  const [facultyConferencesPage, setFacultyConferencesPage] = useState(
    facultyConferencesDetails
  );
  ...
  const handleChangeRelation = (evt, i) => {
    ...
    setFacultyConferencesPage(
      facultyConferencesPage.map((e, index) => {
        if (i == index) {
          return {
            ...e,
  ...
  const addRow = () => {
    setFacultyConferencesPage(prevState => {
      return [
        ...prevState,
        {
          res: {
  ...
  const deleteRowHandle = i => {
    setFacultyConferencesPage(prev => {
  ...
  useEffect(() => {
    setFacultyConferencesPage(facultyConferencesDetails);
  }, [facultyConferencesDetails]);
  return (
    <div>
      <Box>
        <Grid item xs={12} sm={12}>
          <Grid container spacing={2}>
            <Grid item xs={12} sm={12} />
            {facultyConferencesPage.map((relData, index) => {
              return (
                <>
                  ...
                  <Grid item xs={12} sm={3} style={{ marginTop: 23 }}>
                    {index === facultyConferencesPage.length - 1 &&
                      index !== 0 && (
                        <>
                          <Button
                            variant="contained"
