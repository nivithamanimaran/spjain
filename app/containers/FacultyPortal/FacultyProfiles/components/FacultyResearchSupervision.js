...
const FacultyResearchSupervision = ({}) => {
  ...
  const [
    facultyResearchSupervisionPage,
    setFacultyResearchSupervisionPage,
  ] = useState(facultyResearchSupervisionDetails);
  ...
  const handleChangeRelation = (evt, i) => {
    ...
    setFacultyResearchSupervisionPage(
      facultyResearchSupervisionPage.map((e, index) => {
        if (i == index) {
          return {
            ...e,
  ...
  const addRow = () => {
    setFacultyResearchSupervisionPage(prevState => {
      return [
        ...prevState,
        {
          res: {
  ...
  const deleteRowHandle = i => {
    setFacultyResearchSupervisionPage(prev => {
  ...
  useEffect(() => {
    setFacultyResearchSupervisionPage(facultyResearchSupervisionDetails);
  }, [facultyResearchSupervisionDetails]);
  return (
    <div>
      <Box>
        <Grid item xs={12} sm={12}>
          <Grid container spacing={2}>
            <Grid item xs={12} sm={12} />
            {facultyResearchSupervisionPage.map((relData, index) => {
              return (
                <>
                  ...
                  <Grid item xs={12} sm={3} style={{ marginTop: 23 }}>
                    {index === facultyResearchSupervisionPage.length - 1 &&
                      index !== 0 && (
                        <>
                          <Button
                            variant="contained"
