...
const FacultyResearchPublications = ({}) => {
  ...
  const [
    facultyResearchPublicationsDetails,
  ...
  ] = useState(facultyResearchPublicationsDetails);
  ...
  const handleChangeRelation = (evt, i) => {
    ...
    setFacultyResearchPublicationsPage(
      facultyResearchPublicationsPage.map((e, index) => {
        if (i == index) {
          return {
            ...e,
  ...
  const addRow = () => {
    setFacultyResearchPublicationsPage(prevState => {
      return [
        ...prevState,
        {
          res: {
  ...
  const deleteRowHandle = i => {
    setFacultyResearchPublicationsPage(prev => {
  ...
  useEffect(() => {
    setFacultyResearchPublicationsPage(facultyResearchPublicationsDetails);
  }, [facultyResearchPublicationsDetails]);
  return (
    <div>
      <Box>
        <Grid item xs={12} sm={12}>
          <Grid container spacing={2}>
            <Grid item xs={12} sm={12} />
            {facultyResearchPublicationsPage.map((relData, index) => {
              return (
                <>
                  ...
                  <Grid item xs={12} sm={3} style={{ marginTop: 23 }}>
                    {index === facultyResearchPublicationsPage.length - 1 &&
                      index !== 0 && (
                        <>
                          <Button
                            variant="contained"
