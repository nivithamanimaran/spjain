...
const FacultyBook = ({}) => {
  ...
  const [facultyBookDetails, setFacultyBookDetails] = React.useState([
    {
      res: {
        attributes: {
  ...
  const [facultyBookPage, setFacultyBookPage] = useState(facultyBookDetails);
  ...
  useEffect(() => {
    setFacultyBookPage(facultyBookDetails);
  }, [facultyBookDetails]);
  return (
    <div>
      <Box>
        <Grid item xs={12} sm={12}>
          <Grid container spacing={2}>
            <Grid item xs={12} sm={12} />
            {facultyBookPage.map((relData, index) => {
              return (
                <>
                  ...
                  <Grid item xs={12} sm={3} style={{ marginTop: 23 }}>
                    {index === facultyBookPage.length - 1 && index !== 0 && (
                      <>
                        <Button
                          variant="contained"
                          color="error"
